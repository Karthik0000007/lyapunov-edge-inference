...
def analyze_convergence(metrics: Dict[str, List[float]]) -> Dict[str, any]:
    ...
    if final_viol <= 2.0:
        ...
        analysis["recommendation"] = (
            "Training plateaued. Need higher λ learning rate or more epochs."
        )
...
def print_recommendations(analysis: Dict[str, any]) -> None:
    ...
    print(
        f"Recent violations:      {analysis['recent_violation_pct']:.1f}% ± {analysis['recent_violation_std']:.1f}%"
    )
    print(f"Target violations:      {analysis['target_violation_pct']:.1f}%")
    print(f"Improvement rate:       {analysis['improvement_rate_pct']:.1f}%")
    print(
        f"Lambda (λ):             {analysis['initial_lambda']:.3f} → {analysis['final_lambda']:.3f}"
    )
    ...
    if final_viol <= 2.0:
        ...
        print("\nNext steps:")
        print(
            "1. Evaluate agent: python scripts/eval_agent.py --checkpoint checkpoints/ppo_lagrangian"
        )
        ...
        print(
            "   python scripts/train_ppo.py --traces traces/ --constraint-threshold 0.005 --epochs 300"
        )
        ...
        print(
            ...
            % (
                analysis["epochs"]
                * analysis["final_violation_pct"]
                / analysis["target_violation_pct"]
            )
        )
...
def main() -> None:
    ...
    parser.add_argument(
        "--checkpoint", type=Path, required=True, help="Checkpoint directory with metrics"
    )
